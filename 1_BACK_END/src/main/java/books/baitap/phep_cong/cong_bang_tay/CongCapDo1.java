...
public class CongCapDo1 {
    public static void main(String[] args) throws IOException {
        XWPFDocument document = new XWPFDocument();
        FileOutputStream out = new FileOutputStream("ConCapDo1.docx");

        generateAdditionExercises(document, "Bài tập: Phép cộng 2 số có tổng ≤ 10", 240);

        document.write(out);
        out.close();
        document.close();
        System.out.println("✅ Đã tạo xong file Word: ConCapDo1.docx");
    }
    ...
    private static void generateAdditionExercises(XWPFDocument doc, String title, int totalCount) {
        ...
        while (problems.size() < totalCount) {
            ...
            if (a + b <= 10) { // chỉ giữ các bài có tổng <= 10
