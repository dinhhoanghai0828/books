...
public class TruCapDo3 {
    public static void main(String[] args) throws IOException {
        XWPFDocument document = new XWPFDocument();
        FileOutputStream out = new FileOutputStream("TruCapDo3.docx");

        generateSubtractionExercises(document, "Bài tập: Phép trừ số có 2 chữ số với số có 1 chữ số (không nhớ)", 240);

        document.write(out);
        out.close();
        document.close();
        System.out.println("✅ Đã tạo xong file Word: TruCapDo3.docx");
    }
    ...
    private static void generateSubtractionExercises(XWPFDocument doc, String title, int totalCount) {
        ...
        while (problems.size() < totalCount) {
            ...
            // chỉ lấy phép trừ KHÔNG có nhớ => hàng đơn vị a >= hàng đơn vị b
            if ((a % 10) < (b % 10)) {
                continue;
            }
