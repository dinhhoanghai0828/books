...
public class TruCapDo8 {
    public static void main(String[] args) throws IOException {
        XWPFDocument document = new XWPFDocument();
        FileOutputStream out = new FileOutputStream("TruCapDo8_HangDoc.docx");

        generateSubtractionExercises(
                document,
                "Bài tập: Phép trừ 2 số có 2 chữ số trong phạm vi 100 (có nhớ) – Dạng cột dọc",
                240);

        document.write(out);
        out.close();
        document.close();
        System.out.println("✅ Đã tạo xong file Word: TruCapDo8_HangDoc.docx");
    }
    ...
    private static void generateSubtractionExercises(XWPFDocument doc, String title, int totalCount) {
        ...
        while (problems.size() < totalCount) {
            ...
            // CÓ NHỚ => hàng đơn vị của a < hàng đơn vị của b
        ...
        // mỗi trang chỉ để 20 phép tính cột dọc
        int perPage = 20;
