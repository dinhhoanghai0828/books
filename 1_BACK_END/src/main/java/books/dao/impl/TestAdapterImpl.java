...
public class TestAdapterImpl implements TestAdapter {
    ...
    private static final String SQL_GET_TEST = "SELECT C.*, V.ENG AS VOLUME_ENG, V.VI AS VOLUME_VI, V.AUDIO AS AUDIO, V.NUMBER AS NUMBER, B.ENG AS BOOK_ENG FROM CONTENTS C JOIN VOLUMES V ON C.VOLUME_SLUG = V.SLUG INNER JOIN BOOKS B ON B.SLUG = V.BOOK_SLUG WHERE V.IS_LANGUAGE_APPROVED = 1 ORDER BY RAND() LIMIT ?";
    private static final String SQL_GET_TEST_BY_VOLUME_SLUG = "SELECT C.*, V.ENG AS VOLUME_ENG, V.VI AS VOLUME_VI, V.AUDIO AS AUDIO, V.NUMBER AS NUMBER, B.ENG AS BOOK_ENG FROM CONTENTS C JOIN VOLUMES V ON C.VOLUME_SLUG = V.SLUG INNER JOIN BOOKS B ON B.SLUG = V.BOOK_SLUG WHERE V.IS_LANGUAGE_APPROVED = 1 AND V.SLUG = ? ORDER BY RAND() LIMIT ?";
