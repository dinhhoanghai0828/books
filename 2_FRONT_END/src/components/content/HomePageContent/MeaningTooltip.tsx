...
const MeaningTooltip = ({
  ...
  if (meaningEnKeywords.length === 0 || meaningViKeywords.length === 0) {
    return null;
  }

  const isEnglish = /^[a-zA-Z ]+$/.test(window.getSelection()?.toString().trim() || '');

  return createPortal(
    <div style={{ ...TOOLTIP_STYLE, left: position.x, top: position.y }}>
